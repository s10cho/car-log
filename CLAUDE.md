...
**홈 화면 애니메이션이 끝날 때까지 기다린다.** 주행거리는 약 1초간 굴러가므로 텍스트를
너무 일찍 읽으면 중간 숫자를 본다. `settleAnimations(tester)` 를 쓴다.

**화면 아래 내용은 `scrollTo(tester, finder)` 로 스크롤한다.** 홈은 차·점수 → 정비 상태 →
기록 순서로 길어서, 아래쪽 위젯은 빌드되지 않는다. 사용자도 스크롤해야 하는 것이 맞다.
